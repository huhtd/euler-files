...
int main(int argc, char *argv[])
{
    ...
    //  to the top
    MPI_Sendrecv(&data[DOMAINSIZE+1], SUBDOMAIN, MPI_DOUBLE, rank_top, 0, &data[(DOMAINSIZE-1)*DOMAINSIZE+1], SUBDOMAIN, MPI_DOUBLE, rank_bottom, 0, comm_cart, MPI_STATUS_IGNORE);

    //  to the bottom
    MPI_Sendrecv(&data[(DOMAINSIZE-2)*DOMAINSIZE+1], 6, MPI_DOUBLE, rank_bottom, 0, &data[1], 6, MPI_DOUBLE, rank_top, 0, comm_cart, MPI_STATUS_IGNORE);

    //  to the left
    MPI_Sendrecv(&data[DOMAINSIZE+1], 1, data_ghost, rank_left, 0, &data[2*DOMAINSIZE-1], 1, data_ghost, rank_right, 0, comm_cart, MPI_STATUS_IGNORE);
    
    //  to the right
    MPI_Sendrecv(&data[2*DOMAINSIZE-2], 1, data_ghost, rank_right, 0, &data[DOMAINSIZE], 1, data_ghost, rank_left, 0, comm_cart, MPI_STATUS_IGNORE);
